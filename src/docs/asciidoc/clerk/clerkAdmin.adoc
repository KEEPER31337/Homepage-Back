ifndef::snippets[]
:snippets: ./build/generated-snippets
endif::[]
// 자동으로 생성된 snippet 설정하는 부분

= KEEPER API 문서
:icons: font
// NOTE, TIP, WARNING, CAUTION, IMPORTANT 같은 경고구들 아이콘화 해줌
:source-highlighter: highlight.js
// source code 블럭에서 사용되는 highlighter 설정, 4개 정도 있던데 차이를 아직 잘 모르겠음.
:toc: left
// table of contents(toc) 왼쪽정렬하여 생성
:toclevels: 1
// default : 2 (==,  ===) 까지 toc에 보여줌.
:sectlinks:
// section( ==, === ... ) 들을 자기 참조 링크가 있게끔 만들어줌

== API 목록

link:../admin.html[어드민 API 목록으로 돌아가기]

== *수정 가능한 ROLE 목록 불러오기*

=== 요청

==== Request

include::{snippets}/get-job-list/http-request.adoc[]

=== 응답

==== Response

include::{snippets}/get-job-list/http-response.adoc[]

==== Response Field

include::{snippets}/get-job-list/response-fields.adoc[]

== *수정 가능한 TYPE 목록 불러오기*

=== 요청

==== Request

include::{snippets}/get-type-list/http-request.adoc[]

=== 응답

==== Response

include::{snippets}/get-type-list/http-response.adoc[]

==== Response Field

include::{snippets}/get-type-list/response-fields.adoc[]

== *활동 상태별 회원 목록 불러오기*

=== 요청

==== Request

include::{snippets}/get-clerk-member-list-by-type/http-request.adoc[]

==== Path Parameter

include::{snippets}/get-clerk-member-list-by-type/path-parameters.adoc[]

=== 응답

==== Response

include::{snippets}/get-clerk-member-list-by-type/http-response.adoc[]

==== Response Field

include::{snippets}/get-clerk-member-list-by-type/response-fields.adoc[]

== *활동 상태별 회원 목록 불러오기*

=== 요청

==== Request

include::{snippets}/get-clerk-member-list-by-role/http-request.adoc[]

==== Path Parameter

include::{snippets}/get-clerk-member-list-by-role/path-parameters.adoc[]

=== 응답

==== Response

include::{snippets}/get-clerk-member-list-by-role/http-response.adoc[]

==== Response Field

include::{snippets}/get-clerk-member-list-by-role/response-fields.adoc[]

== *직책(ROLE) 등록*

=== 요청

==== Request

include::{snippets}/assign-job/http-request.adoc[]

==== Path Parameter

include::{snippets}/assign-job/path-parameters.adoc[]

==== Request Fields

include::{snippets}/assign-job/request-fields.adoc[]

=== 응답

==== Response

include::{snippets}/assign-job/http-response.adoc[]

==== Response Field

include::{snippets}/assign-job/response-fields.adoc[]

== *직책(ROLE) 삭제*

=== 요청

==== Request

include::{snippets}/delete-job/http-request.adoc[]

==== Path Parameter

include::{snippets}/delete-job/path-parameters.adoc[]

==== Request Fields

include::{snippets}/delete-job/request-fields.adoc[]

=== 응답

==== Response

include::{snippets}/delete-job/http-response.adoc[]

==== Response Field

include::{snippets}/delete-job/response-fields.adoc[]

== *회원의 활동 상태 변경*

=== 요청

==== Request

include::{snippets}/update-member-type/http-request.adoc[]

==== Path Parameter

include::{snippets}/update-member-type/path-parameters.adoc[]

=== 응답

==== Response

include::{snippets}/update-member-type/http-response.adoc[]

==== Response Field

include::{snippets}/update-member-type/response-fields.adoc[]

== *세미나 목록 조회* ==

=== 요청

==== Request

include::{snippets}/get-seminar-list/http-request.adoc[]

=== 응답

==== Response

include::{snippets}/get-seminar-list/http-response.adoc[]

==== Response Field

include::{snippets}/get-seminar-list/response-fields.adoc[]

== *세미나 생성* ==

=== 요청

==== Request

include::{snippets}/create-seminar/http-request.adoc[]

=== 응답

==== Response

include::{snippets}/create-seminar/http-response.adoc[]

==== Response Field

include::{snippets}/create-seminar/response-fields.adoc[]


== *세미나 출석 상태 목록 조회* ==

=== 요청

==== Request

include::{snippets}/get-seminar-attendance-statuses/http-request.adoc[]

=== 응답

==== Response

include::{snippets}/get-seminar-attendance-statuses/http-response.adoc[]

==== Response Field

include::{snippets}/get-seminar-attendance-statuses/response-fields.adoc[]


== *전체 세미나 출석 목록 조회* ==

=== 요청

==== Request

include::{snippets}/get-seminar-attendance-list/http-request.adoc[]

=== 응답

==== Response

include::{snippets}/get-seminar-attendance-list/http-response.adoc[]

==== Response Field

include::{snippets}/get-seminar-attendance-list/response-fields.adoc[]

== *특정 세미나 출석 목록 조회* ==

=== 요청

==== Request

include::{snippets}/get-seminar-attendance/http-request.adoc[]

=== 응답

==== Response

include::{snippets}/get-seminar-attendance/http-response.adoc[]

==== Response Field

include::{snippets}/get-seminar-attendance/response-fields.adoc[]


== *세미나 출석 수정* ==

=== 요청

==== Request

include::{snippets}/update-seminar-attendance/http-request.adoc[]

==== Path Parameter

include::{snippets}/update-seminar-attendance/path-parameters.adoc[]

==== Request Body

include::{snippets}/update-seminar-attendance/request-body.adoc[]

=== 응답

==== Response

include::{snippets}/update-seminar-attendance/http-response.adoc[]

==== Response Field

include::{snippets}/update-seminar-attendance/response-fields.adoc[]